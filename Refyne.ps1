...
function Write-MainMenuStart {
    ...
    PROCESS {
        ...
        if ($WindowsVersion -eq 11) {
            ...
            if (Test-Path $RegistryBackupPath -PathType Leaf) { Write-ColorOutput -InputObject "[1] Run Prolix Tweaks                [5] Revert Changes" -ForegroundColor Gray }
            else { Write-ColorOutput -InputObject "[1] Run Prolix Tweaks" -ForegroundColor Gray }
